...
public interface CombateEntrenadorRepositorio extends JpaRepository<CombateEntrenador, Long> {
    /*
    @Query("SELECT e FROM Entrenador t WHERE t.id NOT IN (SELECT c.idEntrenador1 FROM CombateEntrenador c) AND t.id NOT IN (SELECT c.idEntrenador2 FROM CombateEntrenador c)")
